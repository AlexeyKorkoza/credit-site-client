...
class Editor extends Component {
  ...
  componentDidMount() {
    ...
    if (Object.keys(params).length > 0) {
      ...
      clients.getClient(clientId).then(result => {
        const { client } = result;
  ...
  onDeleteClient = event => {
    ...
    const { clientId, failureNotificationType, successfulNotificationType } = this.state;
  ...
  onMarkClientForDeletion = event => {
    ...
    return clients
      .markClientForDeletion(clientId)
      .then(() => {
        const message = 'Client was deleted successfully';
  ...
  onSave = event => {
    ...
    const func = action === 'edit' ? clients.saveClient(body, +clientId) : clients.saveClient(body);
